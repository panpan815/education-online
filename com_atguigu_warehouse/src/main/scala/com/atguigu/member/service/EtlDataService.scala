  ...
  def etlMemVipLevelLog(ssc: SparkContext, sparkSession: SparkSession) = {
    ...
    ssc.textFile("/user/atguigu/ods/pcenterMemViplevel.log").filter(item => {
      ...
      partition.map(item => {
        ...
        val end_time = jSONObject.getTimestamp("end_time")
        val last_modify_time = jSONObject.getTimestamp("last_modify_time")
        ...
        val start_time = jSONObject.getTimestamp("start_time")
        val vip_id = jSONObject.getIntValue("vip_id")
