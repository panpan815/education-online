...
object DwsMemberService {
  ...
  def importMemberUseApi(sparkSession: SparkSession, dt: String) = {
    ...
    val dwdMemberRegtype = DwdMemberDao.getDwdMemberRegType(sparkSession).drop("dn")
    val dwdBaseAd = DwdMemberDao.getDwdBaseAd(sparkSession).drop("dn")
    val dwdBaseWebsite = DwdMemberDao.getDwdBaseWebSite(sparkSession).drop("dn")
    val dwdPcentermemPaymoney = DwdMemberDao.getDwdPcentermemPayMoney(sparkSession).drop("dn")
    val dwdVipLevel = DwdMemberDao.getDwdVipLevel(sparkSession).drop("dn")
    import org.apache.spark.sql.functions._
    //    dwdMemberRegtype.show(10000)
    val result = dwdMember.join(dwdMemberRegtype, Seq("uid"), "left")
      .join(dwdBaseAd, Seq("ad_id"), "left_outer")
      .join(dwdBaseWebsite, Seq("siteid"), "left_outer")
      .join(dwdPcentermemPaymoney, Seq("uid"), "left_outer")
      .join(dwdVipLevel, Seq("vip_id"), "left_outer")
