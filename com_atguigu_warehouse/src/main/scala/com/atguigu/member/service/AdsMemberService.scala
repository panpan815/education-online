...
object AdsMemberService {
  ...
  def queryDetailSql(sparkSession: SparkSession, dt: String) = {
    ...
    appregurlCount.show()
    top3MemberLevelPayMoneyUser.show()
    sparkSession.sql("select a.uid,a.fullname,b.appkey,b.createtime from dwd.dwd_member a inner join dwd.dwd_member_regtype b" +
      " on a.uid =b.uid and a.dt=b.dt").show()
    ...
    df1.join(df2,Seq("uid","dt"),"left").show()
